...
- The lane slides continuously at about 10 pt/s — a casual drift, not a carousel in a hurry: it never rests on a tile and never rewinds. It renders the release twice, so when its offset passes one full copy it drops by exactly that length and lands on identical pixels; that is what makes the loop endless. The second copy is decoration and must be hidden from assistive technology.
