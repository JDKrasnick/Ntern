...
const labelsPath = option('--labels'); const rawPath = option('--raw-report'); const projectedPath = option('--projected-report'); const outputBase = option('--out'); const includeIdsPath = option('--include-ids');
...
const includeIds = includeIdsPath ? new Set(JSON.parse(await readFile(includeIdsPath, 'utf8')) as string[]) : undefined;
if (!Array.isArray(labels.labels) || labels.labels.length === 0) throw new Error('labels.labels must be non-empty');
const filteredLabels = includeIds ? labels.labels.filter(({ id }) => includeIds.has(id)) : labels.labels;
const filteredRaw = includeIds ? raw.records.filter(({ id }) => includeIds.has(id)) : raw.records;
const filteredProjected = includeIds ? projected.records.filter(({ id }) => includeIds.has(id)) : projected.records;
if (includeIds && filteredLabels.length !== includeIds.size) throw new Error('An included ID has no human label');
const byId = new Map(filteredLabels.map((label) => [label.id, label]));
if (byId.size !== filteredLabels.length || byId.size !== filteredRaw.length || filteredRaw.length !== filteredProjected.length) throw new Error('Labels, raw report, and projected report must have the same unique record count after filtering');
for (const record of filteredRaw) {
...
const rawById = new Map(filteredRaw.map((record) => [record.id, record])); const finalById = new Map(filteredProjected.map((record) => [record.id, record]));
function score(name: 'raw' | 'projected', conditional: boolean) {
  ...
  for (const labelRecord of filteredLabels) {
...
const result = { cohort: labels.cohort, provenance: { labelsPath, rawPath, projectedPath, includeIdsPath, denominator: filteredLabels.length, excluded: labels.labels.length - filteredLabels.length, rawValid: filteredRaw.filter(({ valid }) => valid).length, finalValid: filteredProjected.filter(({ valid }) => valid).length, finalRetainedTags: filteredProjected.reduce((sum, record) => sum + Object.values(record.extraction?.fields ?? {}).filter((field) => field.status === 'present').length, 0), tagsRemovedByEvidenceProjection: projected.tagsRemovedByEvidenceProjection }, raw: { endToEnd: score('raw', false), conditionalOnValid: score('raw', true) }, projected: { endToEnd: score('projected', false), conditionalOnValid: score('projected', true) } };
...
await writeFile(`${outputBase}.md`, `# ${labels.cohort} v14 human-label score\n\nDenominator: ${result.provenance.denominator} roles; excluded: ${result.provenance.excluded}; raw valid: ${result.provenance.rawValid}/${result.provenance.denominator}; final valid: ${result.provenance.finalValid}/${result.provenance.denominator}; final retained tags: ${result.provenance.finalRetainedTags}; tags removed globally by evidence projection: ${JSON.stringify(projected.tagsRemovedByEvidenceProjection)}.\n\n${table('Raw end-to-end', result.raw.endToEnd)}\n${table('Raw conditional on valid response', result.raw.conditionalOnValid)}\n${table('Projected end-to-end', result.projected.endToEnd)}\n${table('Projected conditional on valid response', result.projected.conditionalOnValid)}\n`);
