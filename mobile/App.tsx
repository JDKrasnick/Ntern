    ...
    <ScrollView style={styles.list} contentContainerStyle={styles.resumeContent}>
      ...
      <View style={styles.resumePrimaryTask} onLayout={(event) => setJobTaskHeight(event.nativeEvent.layout.height)}>
        ...
        {fetchingPosting || jobImport?.status === "pending" ? (
          <ResumePostingFetch
            caption={jobImport?.status === "pending"
              ? "Reading the posting. You can wait, or paste it yourself below."
              : "Fetching the posting from the employer’s site…"}
        ...
        {jobImport ? (
          <View style={styles.resumeManualFallback}>
            ...
            <Text style={styles.resumeSectionDescription}>{jobImport.status === "pending" ? "Still reading the public page. Pasting the description skips the wait." : `${resumeImportFailureMessages[jobImport.failureReason ?? ""] ?? "We couldn't read the public page."} Paste the description to continue.`} Pasted text stays in your private resume workspace.</Text>
